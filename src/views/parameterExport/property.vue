<template>
  ...
  <div class="app-container">
    ...
    >
      ...
      <el-table-column
        label="应缴费日期"
        prop="houseShallPayDate"
        align="center"
      />
      <el-table-column label="截止日期" prop="houseDeadline" align="center" />
      <el-table-column
        label="到期验证"
        prop="houseClosingVerify"
        align="center"
      />
      <el-table-column label="逾期天数" prop="houseOverdueDays" align="center">
        <template slot-scope="scope">
          <el-tag
            :type="scope.row.houseOverdueDays >= 0 ? 'danger' : 'success'"
            disable-transitions
            >{{ scope.row.houseOverdueDays }}</el-tag
          >
        </template>
      </el-table-column>
      ...
      >
        <template slot-scope="{ row }">
          ...
          <el-button
            type="primary"
            size="mini"
            @click="handleRangeStart(row.houseId)"
          >
            指定日期
          </el-button>
        </template>
      ...
      >
        <template slot-scope="{ row }">
          ...
          <el-button
            type="primary"
            size="mini"
            @click="handleMoneyGet(row.houseId, row.houseName)"
          >
            收缴
          </el-button>
        </template>
        ...
        <div v-if="payPattern == 1" label="单一缴费" label-width="100px">
          ...
          >
            ...
            <el-tag
              size="large"
              type="info"
              style="width: 100px;text-align:center;margin-left: 100px;margin-bottom:20px;"
              class="payType-item"
              disabled
              >应缴金额</el-tag
            >
            ...
            this.$confirm('确定提交么？', '费用收缴', {
              ...
              singleMoneyPost(singleFormPost).then(response => {
                if (response.codeStatus === 200) {
                  ...
                  this.mixFormPost.remark = ''
                  this.dialogMoneyPost = false
                  fetchSearchByHouseId(singleFormPost.houseId).then(
                    response => {
                      this.tableData = response.data.items
                    }
                  )
